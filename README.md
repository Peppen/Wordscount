...
- Nell'ultima fase ciascuno dei processi invia le proprie mappe locali al processo MASTER che ha solo bisogno di raccogliere tutte queste informazioni che salvà in un file testuale(.txt) con le parole e le frequenze ordinate; i tempi di esecuzione verranno salvati in seguito in un secondo file testuale(.txt).
